...
load_city_parking <- function(progress = NULL) {
  ...
  if (length(zone_objs)) {
    ...
    if (nrow(rate_zones)) {
      idx <- st_intersects(meters, rate_zones)
...
server <- function(input, output, session) {
  ...
  add_unmetered_overlay <- function(proxy, d, radius_m) {
    ...
    dest_geom <- st_sfc(st_point(c(d$lon, d$lat)), crs = 4326)
    search_area <- st_buffer(dest_geom, dist = radius_m)
    hit <- lengths(st_intersects(z, search_area)) > 0
    z2 <- z[hit, ]
    if (!nrow(z2)) return(proxy)
